    ...
    public static partial class Extensions
    {
        ...
        public static dynamic ToDynamic(this DataRow row, string typeName)
        {
            ...
            foreach (DataColumn column in row.Table.Columns)
            {
                ...
                {
                    prop.SetValue(instance, row[column]);
                }
            }
            return instance;
        }
        ...
        {
            return BuildDynamicType(typeName, nameSpace, out assemblyBuilder, persist: false, propertyNames: propertyNames);
        }

        public static Type BuildDynamicType(this string typeName, string nameSpace, out AssemblyBuilder assemblyBuilder, bool persist, params string[] propertyNames)
        {
            lock (_buildDynamicTypeLock)
            {
                ...
                {
                    TypeBuilder typeBuilder;
                    GetAssemblyAndTypeBuilder(fullName, out assemblyBuilder, out typeBuilder, useCache: true, persist: persist);
                    foreach (string propertyName in propertyNames)
                    {
                        AddPropertyToDynamicType(typeBuilder, new PropertyInfoDescriptor(propertyName, typeof(object)));
                    }
        ...
        {
            GetAssemblyAndTypeBuilder(typeName, out assemblyBuilder, out typeBuilder, useCache, persist: false);
        }

        public static void GetAssemblyAndTypeBuilder(string typeName, out AssemblyBuilder assemblyBuilder, out TypeBuilder typeBuilder, bool useCache, bool persist)
        {
            ...
            else
            {
                ...
                if (persist)
                {
                    var persistedBuilder = new PersistedAssemblyBuilder(assemblyName, typeof(object).Assembly);
                    assemblyBuilder = persistedBuilder;
                }
                else
                {
        ...
        /// <summary>
        /// Saves a previously built dynamic assembly to disk. The assembly must have been created with persist=true
        /// via GetAssemblyAndTypeBuilder. The type must have been created (via CreateDynamicType) before calling this method.
        /// </summary>
        /// <param name="typeName">The type name used when building the dynamic type.</param>
        /// <param name="outputPath">The file path to save the assembly to (e.g., "MyDynamic.dll").</param>
        public static void SaveDynamicAssembly(string typeName, string outputPath)
        {
            if (!DynamicTypeStore.Current.ContainsTypeInfo(typeName))
            {
                throw new InvalidOperationException($"No dynamic type info found for '{typeName}'.");
            }

            DynamicTypeInfo info = DynamicTypeStore.Current[typeName];
            if (info.AssemblyBuilder is not PersistedAssemblyBuilder persistedBuilder)
            {
                throw new InvalidOperationException($"Assembly for '{typeName}' was not created with persist=true. Rebuild using GetAssemblyAndTypeBuilder with persist=true.");
            }

            MetadataBuilder metadataBuilder = persistedBuilder.GenerateMetadata(out BlobBuilder ilStream, out BlobBuilder fieldData);
        ...
        public static Type ToDynamicType(this DataRow row, string typeName, out AssemblyBuilder assemblyBuilder)
        {
            lock (_buildDynamicTypeLock)
            {
                ...
                else
                {
                    ...
                    foreach (DataColumn column in row.Table.Columns)
                    {
                        IPropertyInfo propInfo = new PropertyInfoDescriptor(column.ColumnName, row[column].GetType());
        ...
        public static Type ToDynamicType(this Dictionary<string, object> dictionary, string typeName, out AssemblyBuilder assemblyBuilder)
        {
            lock (_buildDynamicTypeLock)
            {
                ...
                else
                {
                    ...
                    {
                        IPropertyInfo propInfo = new PropertyInfoDescriptor(propertyName, dictionary[propertyName].GetType());
                        AddPropertyToDynamicType(typeBuilder, propInfo);
                    }
        ...
        internal static bool PropertyDataTypeFilter(PropertyInfo prop)
        {
            ...
            lock (_buildDynamicTypeLock)
            {
                ...
                foreach (object key in dictionary.Keys)
                {
                    ...
                    else
                    {
                        ...
                        {
                            string childTypeName = "{0}_{1}".Format(typeName, propertyName);
        ...
        private static void ThrowIfLimitReached(int recursionThusFar)
        {
            ...
            {
                throw new InvalidOperationException($"Dynamic type recursion limit of {DynamicTypeInfo.RecursionLimit} reached.");
            }
        }
        ...
        {
            AddPropertyToDynamicType(typeBuilder, new PropertyInfoDescriptor(propertyName, propertyType));
        }
        ...
        internal static void AddPropertyToDynamicType(TypeBuilder typeBuilder, PropertyInfo property)
        {
            AddPropertyToDynamicType(typeBuilder, new PropertyInfoDescriptor(property.Name, property.PropertyType));
        }

        internal static void AddPropertyToDynamicType(TypeBuilder typeBuilder, IPropertyInfo property)
        {
